...
if len(good) > MIN_MATCH_COUNT:
    ...
    img_angle = cv.polylines(img_angle, [np.int32(dst)], True, 255, 3, cv.LINE_AA)
else:
...
draw_params = dict(matchColor=(0, 255, 0),  # draw matches in green color
                   ...
                   flags=2)
img3 = cv.drawMatches(img1, kp1, img_angle, kp2, good, None, **draw_params)
...
if __name__ == '__main__':
    ...
    img_angle = cv.imread(file_name, 0)
    #img_angle = cv.resize(img_angle, size_image)
    # Initiate SIFT detector
    sift = cv.SIFT_create()
    ...
    draw_params = dict(matchColor=(0, 255, 0),  # draw matches in green color
                       ...
                       matchesMask=matchesMask,  # draw only inliers
                       flags=2)
    img3 = cv.drawMatches(img_angle, kp1, img1, kp2, good, None, **draw_params)
    #plt.imshow(cv.cvtColor(img3, cv.COLOR_BGR2RGB)),plt.show()
    plt.imshow(img3, 'gray'), plt.show()
